...
fn test_welcome_screen_shows_on_launch_without_args() {
    ...
    let welcome = session.execute_script(
        r#"
    ...
    assert!(
        welcome_html.contains("preview-md") || welcome_html.contains("Open File") || welcome_html.contains("New File"),
...
fn test_mode_switching_via_toolbar() {
    ...
    let initial = session.execute_script(
        r#"
        const done = arguments[0];
    ...
    let after_click = session.execute_script(
        r#"
        const done = arguments[0];
        done(document.body.dataset.mode);
        "#,
    ...
    assert_eq!(after_mode, "source", "clicking source button should switch to source mode");
    ...
    let final_mode = session.execute_script(
        r#"
        const done = arguments[0];
        done(document.body.dataset.mode);
        "#,
    ...
    assert_eq!(final_mode_str, "preview", "clicking preview button should switch to preview mode");
...
fn test_keyboard_shortcut_ctrl_n_creates_new_file() {
    ...
    let state = session.execute_script(
        r#"
        const done = arguments[0];
...
fn test_file_menu_opens_and_shows_recent_files() {
    ...
    let dropdown = session.execute_script(
        r#"
        const done = arguments[0];
...
fn test_theme_picker_opens_with_ctrl_t() {
    ...
    let picker = session.execute_script(
        r#"
        const done = arguments[0];
