...
| `ui/src/find_preview.test.ts` | Unit tests for the pure helpers exported by `find_preview.ts` (text-node collection, range→match mapping, teardown idempotence). |
| `ui/src/find_source.ts` | Source-side find backend: thin wrappers around the vendored `openSearchPanel`/`findNext`/`findPrevious` + a `searchCompartment` factory consumed by `editor.ts`. |
| `ui/src/find_controller.ts` | Owns the find-bar UI (query input, `n/m` count, prev/next, close, split-mode source⇄preview scope toggle); routes to source or preview backend by scope. |
| `ui/src/frontmatter_edit.ts` | Pure: locate a recognized field's value line, edit a value, add a `key: value` entry, build the insert-new-YAML-block change — all as CodeMirror change descriptors over a doc string + `FrontmatterFact`. |
| `ui/src/frontmatter_edit.test.ts` | Unit tests for locate/edit/add/insert-block round trips, comment preservation, malformed guard. |
...
| `ui/src/codemirror-entry.ts` | Import + re-export `search`, `searchKeymap`, `openSearchPanel`, `findNext`, `findPrevious`. |
| `ui/src/editor.ts` | Add a `searchCompartment` wired into `buildExtensions()`; expose `openSearch`/`findNext`/`findPrevious`/`gotoEditorLine` on `EditorHandle`. |
| `ui/src/main.ts` | Re-point `edit.find/findNext/findPrevious` to the find controller; add `document.editFrontmatter` handling; add a single preview-find recompute call after the render if/else; create the find controller, stats popover, and frontmatter panel; wire chrome controls; map the "Edit frontmatter" diagnostic primary_action to open the inspector. |
| `ui/src/chrome.ts` | Make `.pmd-status-counts` a clickable button (stats popover trigger); add an always-present `.pmd-status-frontmatter` control + `onCountsClick`/`onFrontmatterClick`/`setFrontmatterState` API. |
| `ui/src/mermaid_runner.ts` | Replace the bare `textContent = source` catch with a structured inline error (message + visible source + "Go to source" link calling `gotoEditorLine`). |
| `ui/src/mermaid_zoom.ts` | Add a "Copy source" button next to "Expand" in `addMermaidExpandButton`. |
...
Edit `ui/src/codemirror-entry.ts`. After the `import { unifiedMergeView } from '@codemirror/merge';` line add:
```ts
import {
  search,
  searchKeymap,
...
/** Open CodeMirror's find panel over the given view. */
export function openSourceFind(view: EditorView): void {
  openSearchPanel(view);
}

/** Advance to the next/previous source match (no-ops if the panel is closed). */
export function sourceFindNext(view: EditorView): void {
  findNext(view);
}
export function sourceFindPrevious(view: EditorView): void {
  findPrevious(view);
}
```
...
```
Expected: passes (the bundle re-exports these symbols from Task 0.5).
...
Run:
```
cd ui && npm run typecheck && cd ui && node vendor/build-codemirror.mjs >/dev/null && cd .. && cd ui && npm run build
```
Expected: typecheck passes; app bundle builds without error.
...
test("maps flat ranges to per-node offsets", () => {
...
test("splits a match spanning two nodes into two sub-ranges", () => {
...
/** A highlight sub-range within a single text node. */
 ...
 * spans several chunks produces one sub-range per overlapped chunk. Pure.
 */
export function mapMatchesToNodeRanges(
  ...
  const out: NodeRange[] = [];
  for (const [mFrom, mTo] of matches) {
    for (const chunk of chunks) {
      ...
      if (overlapFrom < overlapTo) {
        out.push({ node: chunk.node, from: overlapFrom - chunk.start, to: overlapTo - chunk.start });
      }
    }
  }
  return out;
...
export function createPreviewFind(root: HTMLElement): PreviewFind {
  ...
  function recompute(): void {
    ...
    ranges = buildRanges();
    current = ranges.length > 0 ? Math.min(Math.max(current, 0), ranges.length - 1) : -1;
    paint();
  }
...
Run:
```
cd ui && node --test src/find_preview.test.ts && cd ui && npm run typecheck
```
...
Run:
```
cd ui && npm run typecheck && cd ui && npm run build
```
Expected: typecheck passes; app bundle builds.
...
### Task 2.1 — `frontmatter_edit.ts` locate-value: failing test

Line semantics (verified against `crates/pmd-core/src/facts/frontmatter.rs` + `crates/pmd-core/tests/document_facts_frontmatter.rs`): `line_start = 1` (opening fence); `line_end` = the **last content line** (line *before* the closing fence); closing fence sits at `line_end + 1`. Unclosed → `line_end = last line of file`, `syntax = "malformed"`. All line numbers are **1-based**.

Create `ui/src/frontmatter_edit.test.ts`:
```ts
import test from "node:test";
import assert from "node:assert/strict";
import type { FrontmatterFact } from "./document_contracts.ts";
import {
  locateFieldLine,
  editValueChange,
  addEntryChange,
...
const valid: FrontmatterFact = {
  format: "yaml",
  line_start: 1,
  line_end: 3, // content lines 2..3; closing fence at line 4
  raw: "---\ntitle: Old\ntags: a, b\n---\n",
  syntax: "valid",
  metadata: {
    title: "Old", description: null, slug: null, sidebar_label: null,
    sidebar_position: null, tags: ["a", "b"], draft: null, unknown: {},
  },
};

const doc = "---\ntitle: Old\ntags: a, b\n---\n# Body\n";

test("locateFieldLine finds a recognized key within the block", () => {
  assert.equal(locateFieldLine(doc, valid, "title"), 2);
  assert.equal(locateFieldLine(doc, valid, "tags"), 3);
});

test("locateFieldLine returns null for an absent key", () => {
  assert.equal(locateFieldLine(doc, valid, "slug"), null);
});
```
...
### Task 2.2 — Implement `locateFieldLine`; pass

Create `ui/src/frontmatter_edit.ts`:
```ts
import type { FrontmatterFact } from './document_contracts.js';
...
const KEY_RE = (key: string) =>
  // YAML "key:" or TOML "key =" at line start, allowing leading spaces.
  new RegExp(`^\\s*${key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*[:=]`);

/**
 * 1-based line number of the recognized `key` within the frontmatter content
 * lines `[line_start+1, line_end]`, or `null` if not present.
 */
export function locateFieldLine(
  doc: string,
  fm: FrontmatterFact,
  key: string,
): number | null {
  const re = KEY_RE(key);
  for (let line = fm.line_start + 1; line <= fm.line_end; line++) {
...
```ts
export function editValueChange(): FmChange { throw new Error('not implemented'); }
export function addEntryChange(): FmChange { throw new Error('not implemented'); }
...
cd ui && node --test src/frontmatter_edit.test.ts
```
Expected: the two `locateFieldLine` tests pass.

Commit:
```
git add -A && git commit -m "feat(frontmatter): locate recognized field line in buffer"
```

### Task 2.3 — `editValueChange`: failing test

Append to `ui/src/frontmatter_edit.test.ts`:
```ts
test("editValueChange replaces only the value text, preserving the key + indent", () => {
  const change = editValueChange(doc, valid, "title", "New Title");
  assert.ok(change);
  const next = doc.slice(0, change.from) + change.insert + doc.slice(change.to);
  assert.equal(next, "---\ntitle: New Title\ntags: a, b\n---\n# Body\n");
});

test("editValueChange returns null for a malformed block", () => {
  const malformed: FrontmatterFact = { ...valid, syntax: "malformed" };
  assert.equal(editValueChange(doc, malformed, "title", "x"), null);
});

test("editValueChange returns null when the key is absent", () => {
  assert.equal(editValueChange(doc, valid, "slug", "x"), null);
});
```
...
cd ui && node --test src/frontmatter_edit.test.ts
```
Expected: FAILS — `editValueChange` throws "not implemented" / returns wrong type.

### Task 2.4 — Implement `editValueChange`; pass

Edit `ui/src/frontmatter_edit.ts`. Replace the `editValueChange` stub with:
```ts
/**
 * Replace the value of a recognized scalar field with `value`, keeping the key,
 * delimiter and leading whitespace. Returns `null` for a non-valid block or an
 * absent key. Writes only to the located line's value span.
 */
export function editValueChange(
  doc: string,
  fm: FrontmatterFact,
  key: string,
  value: string,
): FmChange | null {
  if (fm.syntax !== 'valid') return null;
  const line = locateFieldLine(doc, fm, key);
  if (line === null) return null;
  const from = lineStartOffset(doc, line);
  const to = lineEndOffset(doc, line);
  const text = doc.slice(from, to);
  // Capture "<indent><key><ws><:|=><ws>" prefix; replace the remainder.
  const m = text.match(/^(\s*[^:=]+\s*[:=]\s*)(.*)$/);
  if (!m) return null;
  const prefixLen = m[1].length;
  return { from: from + prefixLen, to, insert: value };
}
```
(Helper `lineStartOffset`/`lineEndOffset` already exist.)

Run:
```
cd ui && node --test src/frontmatter_edit.test.ts
```
Expected: the three `editValueChange` tests + the two locate tests pass.

Commit:
```
git add -A && git commit -m "feat(frontmatter): edit recognized scalar value as line transaction"
```

### Task 2.5 — `addEntryChange`: failing test

Append to `ui/src/frontmatter_edit.test.ts`:
```ts
test("addEntryChange inserts a new key:value line just before the closing fence", () => {
  const change = addEntryChange(doc, valid, "slug", "my-slug");
  assert.ok(change);
  const next = doc.slice(0, change.from) + change.insert + doc.slice(change.to);
  assert.equal(next, "---\ntitle: Old\ntags: a, b\nslug: my-slug\n---\n# Body\n");
});

test("addEntryChange uses '=' for a TOML block", () => {
  const toml: FrontmatterFact = { ...valid, format: "toml" };
  const tomlDoc = "+++\ntitle = \"Old\"\n+++\n# Body\n";
  const tomlFm: FrontmatterFact = { ...toml, line_end: 2, raw: "+++\ntitle = \"Old\"\n+++\n" };
  const change = addEntryChange(tomlDoc, tomlFm, "slug", "my-slug");
  assert.ok(change);
  const next = tomlDoc.slice(0, change.from) + change.insert + tomlDoc.slice(change.to);
  assert.equal(next, "+++\ntitle = \"Old\"\nslug = my-slug\n+++\n# Body\n");
});

test("addEntryChange returns null for a malformed block", () => {
  assert.equal(addEntryChange(doc, { ...valid, syntax: "malformed" }, "slug", "x"), null);
});
```
...
### Task 2.6 — Implement `addEntryChange`; pass

Edit `ui/src/frontmatter_edit.ts`. Replace the `addEntryChange` stub:
```ts
/**
 * Insert a new `key: value` (YAML) / `key = value` (TOML) entry immediately
 * after the last content line (`line_end`), i.e. just before the closing fence
 * at `line_end + 1`. Returns `null` for a non-valid block.
 */
export function addEntryChange(
  ...
  // Insert at the start of the closing-fence line (line_end + 1).
  const at = lineStartOffset(doc, fm.line_end + 1);
  return { from: at, to: at, insert: `${key}${sep}${value}\n` };
}
```
...
cd ui && node --test src/frontmatter_edit.test.ts
```
Expected: all add/edit/locate tests pass.

Commit:
```
git add -A && git commit -m "feat(frontmatter): add-entry line transaction (YAML/TOML)"
```

### Task 2.7 — `insertBlockChange` (no block yet): failing test

Append to `ui/src/frontmatter_edit.test.ts`:
```ts
test("insertBlockChange prepends a new YAML block at the top of the document", () => {
  const bodyDoc = "# Body\n";
...
test("insertBlockChange round-trips through the parser line semantics", () => {
  // The inserted block: line 1 '---', line 2 'title: New' (content), line 3 '---'.
...
cd ui && node --test src/frontmatter_edit.test.ts
```
Expected: FAILS — `insertBlockChange` not implemented.

### Task 2.8 — Implement `insertBlockChange`; pass

Edit `ui/src/frontmatter_edit.ts`. Replace the `insertBlockChange` stub:
```ts
/**
 * Build the change that prepends a new YAML frontmatter block to a document
 * that has none. New blocks always default to YAML per the design.
 */
...
Run:
```
cd ui && node --test src/frontmatter_edit.test.ts && cd ui && npm run typecheck
```
Expected: all frontmatter_edit tests pass; typecheck passes.
...
### Task 2.9 — Add the `document.editFrontmatter` action

Read `ui/src/actions.ts` and `ui/src/actions.test.ts` (counts assertions: `DEFAULT_ACTION_SHORTCUTS` has 22 keys; `NO_DEFAULT_ACTION_IDS.length === 17`). Adding a no-default action bumps `NO_DEFAULT_ACTION_IDS` to 18; the `DEFAULT_ACTION_SHORTCUTS` count stays 22.
...
Update `ui/src/actions.test.ts`: change `assert.equal(NO_DEFAULT_ACTION_IDS.length, 17);` to `assert.equal(NO_DEFAULT_ACTION_IDS.length, 18);`.

Run:
```
cd ui && node --test src/actions.test.ts && cd ui && npm run typecheck
```
Expected: passes (note: the existing test `every registered action has a runnable handler` still passes because `spec()` provides a `run`; the `main.ts` switch must handle the new id — done in Task 2.12 — but the registry test uses a stub `run`).
...
### Task 2.10 — Add the frontmatter chip + stats-button hooks to `chrome.ts`
...
cd ui && npm run typecheck
```
Expected: FAILS — `main.ts` does not yet provide the new handlers? No: adding optional-free interface members does not break existing callers unless they implement the interface elsewhere. The only producer is `createChrome`, the only consumer is `main.ts`; the new members are additive. Typecheck passes.
...
### Task 2.11 — `frontmatter_panel.ts` inspector popover
...
### Task 2.12 — Wire the frontmatter panel + chip + action into `main.ts`
...
import { editValueChange, addEntryChange, insertBlockChange, type FmChange } from './frontmatter_edit.js';
```

2. Add a helper that applies an `FmChange` to the active editor (guarded), near `jumpEditorToBlock` (~line 885):
```ts
function applyFrontmatterChange(change: FmChange | null): void {
  if (!change || !editor) return;
  const max = editor.view.state.doc.length;
  // Guard: a stale locate could exceed the live buffer; bail without mutating.
...
function openFrontmatterInspector(x: number, y: number): void {
  if (!editor) return;
  let fm = activeFrontmatter();
  if (fm === null) {
    // No block yet: insert a new YAML block with a starter title, then re-read.
    applyFrontmatterChange(insertBlockChange(editor.getValue(), 'title', ''));
  }
  fm = activeFrontmatter();
  // NOTE (expected, not a bug): facts refresh only after the backend re-renders,
  // so on the no-block insert path `fm` is still null this turn. The panel then
  // shows the "No frontmatter" header + the add-entry row (the block IS already
  // in the buffer); reopening after the next render shows the recognized fields.
  // Do NOT add a busy-wait — the inserted block is committed; this is cosmetic.
  openFrontmatterPanel(x, y, fm, {
    onEditValue: (key, value) => {
      const cur = activeFrontmatter();
      applyFrontmatterChange(editValueChange(editor!.getValue(), cur!, key, value));
    },
    onAddEntry: (key, value) => {
      const cur = activeFrontmatter();
      applyFrontmatterChange(addEntryChange(editor!.getValue(), cur!, key, value));
    },
  });
}
```
...
### Task 2.13 — Frontmatter panel + chip CSS
...
### Task 3.1 — Structured Mermaid error in `mermaid_runner.ts`

Read `ui/src/mermaid_runner.ts` (the `catch` at lines 118–121: currently `classList.add("pmd-mermaid-error"); container.textContent = source;`). Replace with a structured error built via DOM APIs (createElement/textContent only — preserves the existing sanitization invariant). The "Go to source" link reads `container.dataset.srcStart` (copied on by `copySourceRange`) and calls `gotoEditorLine`, supplied through a setter so `mermaid_runner.ts` keeps no `main.ts` dependency.

Edit `ui/src/mermaid_runner.ts`:

1. After the imports (line 3), add a module-level injectable jump callback:
```ts
...
2. Replace the `catch (e)` body (lines 118–121):
```ts
  } catch (e) {
...
3. Add the renderer at the end of the file:
```ts
...
Commit:
```
git add -A && git commit -m "feat(mermaid): structured inline error with go-to-source link"
```

### Task 3.2 — "Copy source" button in `mermaid_zoom.ts`

Read `ui/src/mermaid_zoom.ts` `addMermaidExpandButton` (lines 12–30). Add a sibling "Copy source" button copying `container.dataset.mermaidSource`. Make it idempotent like the expand button.

Edit `ui/src/mermaid_zoom.ts`. Inside `addMermaidExpandButton`, after the expand button's `container.appendChild(button);` (line 29) and before the closing `}`:
```ts
  if (!container.querySelector(":scope > button.pmd-mermaid-copy")) {
    ...
    copyBtn.addEventListener("click", () => {
      // User-initiated; copies only the document's own diagram source.
...
```
Note: the early `return` guard at line 14 only checks for the expand button; the copy button has its own idempotence guard above, so it is also added on the first call.
...
Commit:
```
git add -A && git commit -m "feat(mermaid): copy-source button beside expand"
```
...
Run:
```
cd ui && node --test src/stats_popover.test.ts && cd ui && npm run typecheck
```
...
2. Add a helper near `activeFrontmatter` (Task 2.12):
```ts
...
- Type `alpha` into `.pmd-find-input`. Assert `.pmd-find-count` text is `1/2` after pressing Enter once (current index 1, total 2) — i.e. `await expect(page.locator('.pmd-find-count')).toHaveText('1/2')`. (On open in split mode the implied scope is preview.)
- Press the next button (`.pmd-find-btn` with title "Next match") / Enter; assert `.pmd-find-count` becomes `2/2`.
- Switch scope to Source (`.pmd-find-scope-btn[data-scope="source"]` click in split mode). Assert CodeMirror's panel appears: `await expect(page.locator('.cm-panels .cm-search')).toBeVisible()` (CM's search panel root) and that `.pmd-find-count` is now empty (source counts come from CM).
- Press `Escape`; assert `.pmd-find-bar` is hidden.

Run:
```
cd ui && npm run build && cd ui && npm run test:e2e:playwright -- document-inspection-find
```
...
- Setup: `installTauriMock(page, { renderHtml: '<article class="pmd-preview"><pre data-src-start="3" data-src-end="3"><code class="language-mermaid">graph TD; A--&gt;</code></pre></article>' })` (an intentionally broken Mermaid source on source line 3). The mock keeps `renderFacts` default.
- New File; type three lines so the editor has line 3, e.g. `line one\nline two\n\`\`\`mermaid\ngraph TD; A-->\n\`\`\`\n`; await render.
- Assert `.pmd-mermaid-error` appears and contains `.pmd-mermaid-error-message` (non-empty text) and `.pmd-mermaid-error-source` containing `graph TD`.
- Click `.pmd-mermaid-error-goto` ("Go to source"). Assert the editor selection moved to line 3: read the active line via `await page.locator('.cm-activeLine')` containing `mermaid` (line 3 is the ```` ```mermaid ```` fence per the fixture's `data-src-start="3"`). (Adjust the typed content so `data-src-start="3"` lands on a recognizable line; assert that line is active.)
- Hover the diagram container; click `.pmd-mermaid-copy` ("Copy source"). Assert the clipboard write was attempted — since Playwright clipboard reads need permissions, instead assert no exception and the button exists; optionally grant clipboard permission via `context.grantPermissions(['clipboard-write'])` in the test and read back with `navigator.clipboard.readText()` to assert it equals the diagram source.
...
- **Find approach:** Pure `findMatches` (case-insensitive, non-overlapping offset ranges) is unit-tested first. Source find reuses CodeMirror's panel/counts via a new `searchCompartment` + `searchKeymap` in `editor.ts`, driven by the existing `edit.find/findNext/findPrevious` actions (re-pointed in `main.ts`; the global hotkey handler intercepts Ctrl+F, so the action must explicitly open the panel). Preview find uses range-mapped highlights with a tracked current match; a **single** recompute call sits after the render if/else in `processRenderQueue`, covering both reconcile and full-replace branches. The controller hosts the find bar with a split-mode source⇄preview scope toggle and `n/m` counts.

- **Frontmatter approach:** Pure `frontmatter_edit.ts` builds line-oriented `FmChange` descriptors over the buffer using the verified line semantics (`line_start=1`, `line_end` = line before closing fence at `line_end+1`): edit-value replaces only the value span, add-entry inserts before the closing fence, insert-block prepends a new YAML block when none exists. All edits go through the open in-scope CodeMirror buffer (undo/save/render reuse, no new path authority); malformed/unsupported blocks disable add+edit. The inspector popover (context-menu-style) and the always-present `.pmd-status-frontmatter` chip ("frontmatter" / subdued "+ frontmatter") are wired in `main.ts`.

- **Mermaid approach:** The existing `catch` is enhanced (no new render path) to show message + visible source + a "Go to source" button reading `container.dataset.srcStart` and calling the new `gotoEditorLine(line)` on `EditorHandle` (injected into `mermaid_runner.ts` via a setter to avoid a `main.ts` dependency). A "Copy source" button is added beside "Expand" in `mermaid_zoom.ts`, copying `dataset.mermaidSource`. All DOM is built via createElement/textContent — no markup injection.
...
- **Testing:** Unit tests precede wiring in every feature (find query/preview-mapping, frontmatter locate/edit/add/insert, stats rows/reading-time, updated actions inventory). e2e (`document-inspection-find.spec.cjs`) covers find source/preview/split-scope, stats values, frontmatter chip add/edit reflected in source, malformed diagnostic → read-only inspector, and Mermaid inline error → go-to-source + copy-source, following the `installTauriMock`/`appUrl` patterns in `helpers.cjs`. Final gate is `just check` (cargo `-j 2`).

- **Total: 6 phases, 42 tasks.** Each task is one bite-sized TDD step ending in a conventional commit.
